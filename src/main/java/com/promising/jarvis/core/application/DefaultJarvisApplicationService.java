...
/** Coordinates asynchronous parsing and capability dispatch. */
public final class DefaultJarvisApplicationService implements JarvisApplicationService {
    ...
    public DefaultJarvisApplicationService(NLParser parser, CapabilityRegistry capabilities) {
        this.parser = parser;
    ...
    private ContentResponseBody parse(CommandContext context) {
        try { return parser.parse(context.request().text(), context.player().asPromptText()); }
    ...
    private void complete(CommandContext context, ContentResponseBody response, Throwable error) {
        if (error != null || response == null) {
            ...
            context.source().sendError(Text.of("Jarvis 请求失败：" + (cause == null || cause.getMessage() == null ? "未知错误" : cause.getMessage())));
            return;
        }
